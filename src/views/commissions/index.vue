...
export default {
  ...
  methods: {
    async fetchData() {
      this.loading = true
      let { data } = await this.$api.get("/structure/commission/list/")
      this.commissionList = data
      this.loading = false
      console.log(data)
    },
    async removeItem(item) {
      await this.$api.delete("/structure/commission/delete/" + item.id + "/")
      this.fetchData()
    },
    async onSave(item) {
      await this.$api.patch(`/structure/commission/update/${item.id}/`, {
