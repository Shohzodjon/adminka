<template>
  <a-row>
    ...
    <a-table
      v-else-if="subdistricts.results.length"
      :key="$route.fullPath"
      :columns="columns"
      :data-source="subdistricts.results"
      :loading="loading"
...
export default {
  ...
  methods: {
    ...
    async getAllSubDistrict() {
      ...
      this.subdistricts = await this.$store.dispatch(
      ...
      pagination.total = this.subdistricts.count
      this.pagination = pagination
      if (this.subdistricts) {
        this.$set(this, "tableProps", {
          results: (this.subdistricts && this.subdistricts.results) || [],
          count:
            this.subdistricts &&
            this.subdistricts.results &&
            this.subdistricts.results.length
        })
      }
      this.loading = false
    },
    ...
    async getSubDistricts(params) {
      ...
      this.page = +this.$route.query.page || 1
      this.subdistricts = await this.$store.dispatch(
        "subdistrict/fetchSubDistrictList",
        {
          limit: 10,
          offset: this.offset,
    ...
    async removeItem(id) {
      await this.$api.delete(`/common/sub-district/delete/${id}/`)
      this.isFilter
        ? this.getSubDistricts(this.districtId)
        : this.getAllSubDistrict()
    },
