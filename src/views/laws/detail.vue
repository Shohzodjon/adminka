    ...
    <a-col v-show="!loading" class="height100 flex-1">
      ...
      >
        <a-tab-pane key="content">
          <span slot="tab"> <a-icon type="project" />{{ $t("Content") }} </span>
          <!-- lang tabs -->
          <a-row
            align="middle"
            class="flex-baseline"
...
export default {
  ...
  methods: {
    ...
    async removeItem() {
      try {
        this.$api.delete(`/common/laws/delete/${this.$route.params.id}/`)
        this.$router.go(-1)
      } catch (e) {
        console.log(e)
    ...
    async updateLaws() {
      ...
      const date = this.form.date._d
        ? new Date(this.form.date._d.toString())
      ...
      try {
        ...
        const res = await this.$api.patch(
          `/common/laws/update/${this.$route.params.id}/`,
          form
        )
        this.ISVALID(res, () => {
        ...
        this.pending = false
        this.$message.success("Закон успешно создан")
        this.$router.push("/laws")
      } catch (e) {
        console.log(e)
        this.pending = false
