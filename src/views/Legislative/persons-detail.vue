      ...
      <a-tabs class="main-tabs" type="card">
        <a-tab-pane key="content" class="main-tabs" type="card">
          ...
          <a-row align="middle" class="flex-baseline">
            ...
            <a-col :span="12" class="sm-mb">
              <a-input v-model="form.phone" type="tel" placeholder="+998" />
            </a-col>
          </a-row>
          <a-row align="middle" class="flex-baseline">
            <a-col :span="6" style="text-align: right; padding-right: 8px">
...
export default {
  ...
  methods: {
    async onSubmit() {
      ...
      let f = new FormData()
      Object.keys(form).forEach((k) => {
        form[k] && k != "img" && f.append(k, form[k])
        form[k] && k == "img" && f.append(k, form[k], form[k].name)
      })
      const res = await this.$api.patch(
        `/structure/staff/update/${this.$route.params.id}/`,
        f
      )
      this.ISVALID(res, () => {
        this.$router.push({
