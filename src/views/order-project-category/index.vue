  ...
  <a-row>
    <a-row class="content-title" type="flex">
      ...
      <a-row class="flex-no__wrap wanted-search-input-wrapper" type="flex">
        <a-button
          type="primary"
          @click="$router.push({ name: 'order-project-category-create' })"
        >
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      const form = await this.$store.dispatch("orderProjectCategory/fetchList")
      this.form = form
    },
    async removeItem(id) {
      this.loading = true
      try {
        const r = await this.$api.delete(
          `/structure/orderprojectcategory/delete/${id}/`
        )
        if (r) {
          this.fetchData()
