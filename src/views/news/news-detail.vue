        ...
        <a-tab-pane key="content">
          ...
          <a-row style="margin-bottom: 15px">
            ...
            <a-col :span="12">
              ...
              >
                <a-select-option
                  v-for="category in $store.getters['category/list']"
                  :key="category.id"
                  :value="category.id"
                >
                  {{ category.title }}
          ...
          <a-row align="middle">
            ...
            <a-col :span="12">
              <image-upload
                ...
                @set-main-image="main_image = arguments[0]"
              />
            </a-col>
          </a-row>
        </a-tab-pane>
...
export default {
  ...
  created() {
    this.form = {
      ...
      created: this.moment.utc(new Date(), "DD-MM-YYYY")
    }
  ...
  methods: {
    ...
    async submitNews({ noRouterBack = false }) {
      ...
      try {
        ...
        newData = {
          ...newData,
          title: JSON.stringify(newData.title),
          content: JSON.stringify(newData.content),
          created: this.moment.utc(newData.created).format(),
          tags:
    ...
    fetchData() {
      ...
      this.$api
        .get("/news/post/detail/" + this.$route.params.id)
        .then(async (res) => {
          if (res && res.data) {
            // skip non important keys
            // eslint-disable-next-line no-unused-vars
            const { category, created, updated, views, ...data } = res.data
            ...
            this.form = {
              ...
              created: this.moment.utc(created),
              tags:
                (data.tag_list &&
