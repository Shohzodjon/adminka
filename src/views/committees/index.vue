<template>
  <div class="committees">
    ...
    >
      ...
      <template slot="committee_title" slot-scope="item">
        <router-link
          :to="{
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          key: "title",
          scopedSlots: { customRender: "committee_title" },
          width: "70%"
        },
  ...
  methods: {
    async fetchData() {
      this.loading = true
      let { data } = await this.$api.get("/structure/committee/list/")
      this.committeeList = data
      this.loading = false
      //console.log(data)
    },
    async removeItem(item) {
      await this.$api.delete("/structure/committee/delete/" + item.id + "/")
      this.fetchData()
    },
    async onSave(item) {
      await this.$api.patch(`/structure/committee/update/${item.id}/`, {
