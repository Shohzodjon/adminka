  ...
  <div class="fraction-create">
    ...
    <div class="fraction-create-body">
      ...
      <a-row align="top" class="input-form__group" type="flex">
        ...
        <a-col :span="12">
          <a-select v-model="form.project_type" style="width: 100%">
            <a-select-option
      ...
      <a-row align="top" class="input-form__group" type="flex">
        ...
        <a-col :span="12">
          <a-select v-model="form.initiator" style="width: 100%">
            <a-select-option
              v-for="item in initiator_list"
              :key="item.id"
              :value="item.id"
            >
              {{ item.title }}
      ...
      <a-row align="top" class="input-form__group" type="flex">
        ...
        <a-col :span="12">
          <input
            v-model="form.start_date_discussion"
            :class="{
              invalid:
      ...
      <a-row align="top" class="input-form__group" type="flex">
        ...
        <a-col :span="12">
          <input
            v-model="form.end_date_discussion"
            :class="{
              invalid:
...
export default {
  ...
  created() {
    if (this.$route.params && this.$route.params.id != "create") {
      ...
      this.$router.push({ name: "order-project-create" })
    }
  },
  methods: {
    openStoreModal() {
    ...
    async submitFraction() {
      ...
      if (this.$v.$invalid) {
        ...
        const res = await this.$api.patch(
          `/structure/orderproject/update/${this.$route.params.id}/`,
          f
        )
        this.ISVALID(res, () => {
          this.$router.push({
    ...
    async fetchDetail() {
      ...
      let res = await this.$store.dispatch("orderProject/fetchDetail", id)
      // console.log(res)
      this.form = res
      this.form = {
        ...res,
