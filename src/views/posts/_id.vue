    ...
    <a-col v-show="!loading" class="height100 flex-1">
      ...
      >
        ...
        <a-tab-pane key="comments">
          <span slot="tab"> <a-icon type="project" />{{ $t("Comment") }} </span>

          <a-row
            class="flex-no__wrap flex-end ml-auto wanted-search-input-wrapper"
            type="flex"
          >
            <!-- <searchInput
...
export default {
  ...
  watch: {
    ...
    inputTags(newVal) {
      this.selectedTags = newVal.map((el) => {
        return this.tagList.find((tag) => tag.title == el).id
      })
      console.log(newVal, this.tagList, newVal)
    },
  ...
  async created() {
    ...
    this.loading = true
    this.commentsList = await this.$store.dispatch("posts/fetchPostComments", {
      id: this.$route.params.id
    })
  ...
  mounted() {
    ...
    setTimeout(() => {
      console.log(this.document_files_array, "this.document_files_array")
    }, 2000)
  },
  ...
  methods: {
    ...
    handleChange(e) {
      ...
      this.tagList.forEach((item) => {
        e.includes(item.title) && this.form.tags.push(item.id)
      })
    ...
    async deletePost() {
      try {
        ...
        if (res && res.status == 204) {
          ...
          // }
          this.$router.push({ name: "posts", params: { id: this.form.menu } })
        }
      } catch (e) {
    ...
    async getTagList() {
      let res = await this.$api.get("/cms/post/tag/list/")
      ...
      try {
        ...
        newData = {
          ...
          short_content_ru: newData.short_content.ru
            ? newData.short_content.ru
            : " ",
          short_content_uz: newData.short_content.uz
            ? newData.short_content.uz
            : " ",
          short_content_oz: newData.short_content.oz
            ? newData.short_content.oz
            : " ",
          short_content_en: newData.short_content.en
            ? newData.short_content.en
            : " ",
          short_content_qr: newData.short_content.qr
            ? newData.short_content.qr
            : " ",
          content_ru: newData.content.ru ? newData.content.ru : " ",
          content_uz: newData.content.uz ? newData.content.uz : " ",
          content_oz: newData.content.oz ? newData.content.oz : " ",
          content_en: newData.content.en ? newData.content.en : " ",
          content_qr: newData.content.qr ? newData.content.qr : " ",
          title_ru: newData.title.ru ? newData.title.ru : " ",
          title_uz: newData.title.uz ? newData.title.uz : " ",
          title_oz: newData.title.oz ? newData.title.oz : " ",
          title_en: newData.title.en ? newData.title.en : " ",
          title_qr: newData.title.qr ? newData.title.qr : " ",
    ...
    fetchData() {
      ...
      this.$api
        .get("/cms/post/" + this.$route.params.id + "/update/")
        .then(async (res) => {
          if (res && res.data) {
            // skip non important keys
            // eslint-disable-next-line no-unused-vars
            const { category, pub_date, updated, comments, views, ...data } =
              res.data
            this.form = {
              ...this.form,
              ...data,
            ...
            this.selectedTags = data.tags.map((el) => el.id)
            // console.log("form ", this.form)
            // console.log(this.form, this.selectedTags, this.tagList)
            // this.inputTags = this.selectedTags.map((el) => {
            //   return this.tagList.find((tag) => tag.id == el.id).title
            // })
    ...
    async handleImages(arg) {
      ...
      let res = await this.$api.patch(
        "/cms/post/" + this.$route.params.id + "/update/",
        f
      )
      // this.image_List = res && res.data && res.data.images
      if (res && res.data) {
