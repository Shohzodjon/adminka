  ...
  <a-row>
    <a-row class="content-title" type="flex">
      ...
      <a-row class="flex-no__wrap wanted-search-input-wrapper" type="flex">
        <a-button
          type="primary"
          @click="$router.push({ name: 'usefullLinks-create' })"
        >
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      this.loading = true
      const form = await this.$store.dispatch("usefullLinks/fetchUsefulLinks")
      this.form = form
      this.loading = false
    },
    async removeItem(id) {
      this.loading = true
      try {
        const r = await this.$api.delete(`/common/useful-links/delete/${id}/`)
        if (r) {
          this.fetchData()
    ...
    async onSave(item) {
      await this.$api.patch(`/common/useful-links/update/${item.id}/`, {
