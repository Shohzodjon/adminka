<template>
  <div class="fraction-create">
    ...
    <div class="fraction-create-body">
      ...
      <a-row align="top" class="input-form__group" type="flex" style="margin-top: 25px">
        <a-col :span="6" style="text-align: right; padding-top: 5px;padding-right:8px ">
          {{ $t('FileSelect') }} :
        </a-col>
        <a-col :span="6" style="text-align: left; padding-top: 5px; ">
...
export default {
  ...
  created() {
    if (this.$route.params && this.$route.params.id != "create") {
      ...
      this.$router.push({ name: "deputy-requests-create" })
    }
  },
  methods: {
    openStoreModal() {
    ...
    async submitFraction() {
      ...
      if (this.$v.$invalid) {
        ...
        const res = await this.$api.patch(
          `/structure/orderproject/deputy-request/update/${this.$route.params.id}/`,
          f
        )
        this.ISVALID(res, () => {
          this.$router.push({
    ...
    async fetchDetail() {
      ...
      let res = await this.$store.dispatch("deputyRequests/fetchDetail", id)
      // console.log(res)
      this.form = res
      this.form = {
        ...res,
