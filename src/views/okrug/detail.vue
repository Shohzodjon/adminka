<template>
  <div class="fraction-create">
    ...
    <!-- lang tabs -->
    <a-row align="middle" class="flex-baseline" style="margin-top: 50px">
  ...
  methods: {
    ...
    async updateOkrug() {
      ...
      try {
        const res = await this.$api.patch(
          `/common/okrug/${this.$route.params.id}/update/`,
          this.form
        )
        this.ISVALID(res, () => {
          this.$router.go(-1)
    ...
    async getOkrugById() {
      ...
      this.form = {
        ...
        sub_district: form.sub_district.map((i) => i.id)
      }
      console.log(this.form)
