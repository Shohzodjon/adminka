...
export default {
  ...
  created() {
    if (this.$route.params && this.$route.params.id != "create") {
      ...
      this.$router.push({ name: "deputy-projects-create" })
    }
  },
  methods: {
    openStoreModal() {
    ...
    async submitFraction() {
      ...
      const res = await this.$api.put(
        `/structure/orderproject/deputy-initiative/update/${this.$route.params.id}/`, f
      )
      this.ISVALID(res, () => {
        this.$router.push({
    ...
    async fetchDetail() {
      ...
      let res = await this.$store.dispatch("deputyProjects/fetchDetail", id)
      // console.log(res)
      this.form = res
      this.form = {
        ...res,
    ...
    async getLeaderList() {
      this.allDeputies = await this.$store.dispatch(
        "deputyProjects/fetchCommissionDeputies"
      )
    },
  }
}
