<!-- deputat tashabbuskor qonun loyihalari  -->
<template>
    <a-row>
      <a-row class="content-title" type="flex">
        ...
        <a-row class="flex-no__wrap wanted-search-input-wrapper" type="flex">
          ...
          <a-button
            type="primary"
            @click="$router.push({ name: 'deputy-projects-create' })"
          >
  ...
  export default {
    ...
    methods: {
      ...
      async fetchData() {
        const form = await this.$store.dispatch("deputyProjects/fetchList")
        this.form = form;
      ...
      async removeItem(id) {
        ...
        try {
          const r = await this.$api.delete(
            `/structure/orderproject/deputy-initiative/delete/${id}/`
          )
          if (r) {
            this.fetchData()
