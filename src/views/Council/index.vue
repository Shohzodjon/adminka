...
export default {
  ...
  methods: {
    ...
    async getCouncil() {
      console.log("ggg")
      const { data } = await this.$api.get("/structure/council/1/update/")
      this.form = data
        ? data
        : {
            speaker: null,
    ...
    async onSubmit() {
      try {
        this.method == "post"
          ? await this.$api.post("/structure/council/create/", this.form)
          : await this.$api.patch("/structure/council/1/update/", this.form)
