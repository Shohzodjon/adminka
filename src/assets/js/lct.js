...
export const t$ = {
  ...
  _isEqual: function(left, right, ignoreCase) {
    ...
    if (right != null) {
      if (ignoreCase) return left.toLowerCase() == right.toLowerCase();
  ...
  getChar: function(s, i, len) {
    if (len == undefined) len = s.length;
    if (i < len && i > -1) return s[i];
  ...
  getWord: function(s, i, len) {
    ...
    var j = i + 1;
    while (j-- > 0) {
      if (!this.isLetter(this.getChar(s, j, len))) break;
    }
    var w_start = j;
    j = i - 1;
    while (++j < len) {
  ...
  convert: function(s) {
    if (s == null || s == undefined) throw new Error("argument is null");
    ...
    while ((c = this.getChar(s, k, len))) {
      ...
      if (lc === "a") n = "а";
        ...
        if (
          this._isEqual("e", nc, true) &&
          this.isApostrophe(this.getChar(s, k + 1, len))
        )
          n = "э";
        else if (nc != null) {
          if (
            ((nc = nc.toLowerCase()) >= "a" && nc <= "z") ||
        ...
        if (this._isEqual("s", this.getChar(s, k + 1, len), true)) {
          ...
          if (word != null) {
            ...
            hasUzbekOnlyLetters = this._containsAnyOf(
              translated,
              this._uzbLetters
            );
          }
          ...
          if (!hasUzbekOnlyLetters) {
            ...
            // 5. 'ts'+['b','d','f','g','j','k','l','m','n','p','r','s','t','v','x']
              ...
              this._inArray(this._TS_postFixes, this.getChar(s, k + 2, len))
            ) {
              n = "ц";
              k++;
            }
